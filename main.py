    ...
    if 'items' in search_results:
        if search_type == 'image':
            ...
            for i, item in enumerate(search_results['items'], start=start_index):
                try:
                    media = InputMediaPhoto(media=item['link'], caption=f"{i}. [Image]({item['link']})",
            ...
            if start_index > 1:
                buttons.append(
                    InlineKeyboardButton("Previous", callback_data=f"prev_{query}_{start_index}_{search_type}"))
            ...
            await bot.edit_message_text(result_message, chat_id, message_id, parse_mode=types.ParseMode.HTML,
...
async def search(message, bot):
    ...
    if query:
        ...
        try:
            search_results_message = await send_search_results(bot, message.chat.id, sent_message.message_id, query, 1,
                                                               search_type)
        except CantParseEntities:
            search_results_message = await message.reply(
                "<b>⚠️ Google SafeSearch is on, so no results , join @bdbots </b>", parse_mode=types.ParseMode.HTML)
...
async def process_callback_query(callback_query, bot):
    ...
    await send_search_results(bot, callback_query.message.chat.id, callback_query.message.message_id, query,
                              start_index, search_type)
